...
class MonthlyLevelFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    class Meta:
        model = db_models.DailyLevel
        sqlalchemy_session = db_session

    station_number = None
    # start at 2017, increase another year after every 12th instance
    year = factory.Sequence(lambda n: 2011 + n // 12)
    # always between 1-12, even n increases past 12.
    ...
    monthly_mean = factory.Sequence(
        lambda n: (BASE_FLOWS[n % 12] + (0.1 * (n//12))))
    monthly_total = factory.Sequence(lambda n: (
        BASE_FLOWS[n % 12] + (0.1 * (n//12))) * 30)
    min = factory.Sequence(lambda n: (
        BASE_FLOWS[n % 12] + (0.1 * (n//12))) - 0.5)
    max = factory.Sequence(lambda n: (
        BASE_FLOWS[n % 12] + (0.1 * (n//12))) + 0.5)
    station = None
...
class MonthlyFlowFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    # the map statements increase the flow by 10% every 12 months. This allows us to have
    # some variation in yearly data for visualization/testing.
    monthly_mean = factory.Sequence(lambda n: (
        BASE_FLOWS[n % 12] + (0.1 * (n//12))) * 10)
    monthly_total = factory.Sequence(lambda n: (
        BASE_FLOWS[n % 12] + (0.1 * (n//12))) * 300)
    min = factory.Sequence(lambda n: (
        BASE_FLOWS[n % 12] + (0.1 * (n//12))) * 10 - 5)
    max = factory.Sequence(lambda n: (
        BASE_FLOWS[n % 12] + (0.1 * (n//12))) * 10 + 5)
    station = None
...
class StationFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    station_number = factory.Faker(
        'password', length=8, special_chars=False, digits=True, upper_case=True, lower_case=False)
    ...
    geom = factory.LazyAttribute(lambda p: WKTElement(
        Point(p.longitude, p.latitude).wkt, srid=4326))

    monthly_flows = factory.RelatedFactoryList(
        MonthlyFlowFactory, 'station', size=24)
    monthly_levels = factory.RelatedFactoryList(
        MonthlyLevelFactory, 'station', size=24)
