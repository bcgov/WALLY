"""add Port Moody burned dem

Revision ID: 1ce2c58cadfe
Revises: e10c8d94b5ec
Create Date: 2021-07-28 13:11:07.773863
...
def upgrade():
    ...
    # bounds of Burned_PortMoody_1m_4326.tif as outputted by gdalinfo
    bounds = MultiPolygon([Polygon(
        (
            (49.4000108, -123.0000154),
            (49.1991458, -123.0000154),
            (49.4000108, -122.3987965),
            (49.1991458, -122.3987965),
            (49.2995783, -122.6994060),

        )
    )])

    tile = StreamBurnedCDEMTile(
        filename="Burned_PortMoody_1m_4326.tif",
        z_precision=1,
        resolution=1,
        geom='SRID=4326;' + bounds.wkt
    )
    session.add(tile)
    session.commit()
