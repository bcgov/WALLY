...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('update_date', sa.DateTime, comment='Date and time (UTC) when the physical record was updated in the database. It will be the same as the create_date until the record is first updated after creation.'),
    ...
    op.create_table(
        ...
        sa.Column('model_output_type', sa.String, sa.ForeignKey('modeling.model_output_type_code.model_output_type_code'),
        ...
        sa.Column('potential_evapo_transpiration_co', sa.Numeric, comment='A measure of the ability of the atmosphere to remove water through Evapo-Transpiration (ET) processes. A reference crop under optimal conditions, having the characteristics of well-watered grass with an assumed height of 12 centimeters, a fixed surface resistance of 70 seconds per meter and an albedo of 0.23.'),
        ...
        sa.Column('r2', sa.Float, comment='The proportion of the variance for a dependent variable thats explained by an independent variable or variables in a regression model'),
        sa.Column('adjusted_r2', sa.Float,
                  comment='The correlation strength of additional variables.'),
        sa.Column('steyx', sa.Float, comment='Standard error in the estimate of the hydrological variable (Y) as a function of the regression model (X).'),
