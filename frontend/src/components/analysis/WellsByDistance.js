...
export default {
  ...
  computed: {
    ...
    coordinatesArr () {
      let arr = this.coordinates.split(',')
  ...
  methods: {
    ...
    populateBoxPlotData (wells) {
      ...
      wells.forEach(well => {
        ...
        // this.boxPlotData.push(well.well_yield)
      })
    ...
    fetchWells: debounce(function () {
      ...
      const params = {
        ...
        point: JSON.stringify(this.coordinatesArr)
      }
    ...
    showCircle () {
      ...
      const shape = circle(this.coordinatesArr, radius, options)
