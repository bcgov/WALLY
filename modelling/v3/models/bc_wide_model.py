...
for filename in sorted(os.listdir(directory)):
    if filename.endswith(".csv"):
        ...
        training_data = pd.concat([training_data, zone_df])
...
features_df = training_data[df_inputs]
...
for train_index, test_index in kf.split(X):
    ...
    output_dir = "./outputs/bc_wide_model/"
    ...
    # ax1.xlim([0, xabs_max])
    # ax1.ylim([0, yabs_max])
    # ax1.axis((0,xabs_max,0,yabs_max))
    ax1.legend()
...
# pyplot.show()
fig1.savefig(output_dir + "bc_wide_model_summary.png", bbox_inches='tight')
# ----------------
...
model.save_model('../model_output/bc_wide_model/bc_wide_model.json')
...
with open('../model_output/bc_wide_model/annual_model_scores.json', "w") as outfile:
