...
existing_stations_list = existing_stations_df["id"].tolist()


for index, station in stations_df.iterrows():
    if station["id"] not in existing_stations_list:
        print(
          "https://wally-staging.apps.silver.devops.gov.bc.ca/api/v1/watersheds/" + str(station["id"]) + '?generated_watershed_id=' + str(station["gen_id"])
          )